...
public interface InscriptionRepository extends CrudRepository<Inscription, Integer> {
    ...
    @Query("""
            ...
            FROM `inscription`
              LEFT OUTER JOIN `equipe` `equipe` ON `equipe`.`equipe_id` = `inscription`.`inscription_id`
              LEFT OUTER JOIN `competition` `competition` ON `competition`.`competition_id` = `inscription`.`inscription_id`
              LEFT OUTER JOIN `jeu` `competition_jeu` ON `competition_jeu`.`jeu_id` = `competition`.`competition_id`
              LEFT OUTER JOIN `tournois` `competition_tournois` ON `competition_tournois`.`tournois_id` = `competition`.`competition_id`
